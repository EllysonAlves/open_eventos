...
class Login
{
    ...
    public function cadastro($nome, $sobrenome, $email, $senha, $senhaConf,$role)
    {


        if (empty($nome) || empty($sobrenome) || empty($email) || empty($senha) || empty($senhaConf) || empty($role)) {
            // campo vazio
        } else {
            ...
            if (count($dados) > 0) { // email ja existe
                ...
                if ($senha == $senhaConf) { //confirma senha
                    ...
                    $sql = "INSERT INTO usuario (nome,sobrenome,email,senha,role) VALUES (?,?,?,?,?)";
                    $stmt = Conexao::getConexao()->prepare($sql);
